...
public class LibraryRepository {
    ...
    // 도서들의 정보
    private static Book[] bookList; // 모든 책의 정보 담기
    ...
    public String[] searchBookInfoList(String keyword) {
        ...
        for (Book book : bookList) {
            String title = book.getTitle(); // 책 개당 제목
    ...
    public String giveBook(int bilIndex) {
        int addCoupon = bookUser.getCouponCount();
        if (bookList[bilIndex] instanceof CookBook) {
            if (((CookBook) bookList[bilIndex]).isCoupon()) {
                bookUser.setCouponCount(addCoupon+1);
                return String.format("대여 완료! 요리학원 쿠폰이 발급되었으니 마이페이지에서 확인하세요");
            } else {
                bookUser.setCouponCount(addCoupon);
            }
