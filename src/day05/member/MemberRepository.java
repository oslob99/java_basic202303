...
public class MemberRepository {
    ...
    boolean showMember(String findEmail){
        for (int i = 0; i < memberList.length; i++) {
            if (findEmail.equals(memberList[i].email)) {
                System.out.println("# 이름: "+memberList[i].memberName);
                System.out.println("# 비밀번호: "+memberList[i].password);
                System.out.println("# 성별: "+memberList[i].gender);
                System.out.println("# 나이: "+memberList[i].age);
                return true;
            }
        }
        return false;
